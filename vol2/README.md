...
第4章で講師の環境の実物と、柵を足すときの考え方4つを見せる。全35枚。テーマCSSは第1回の`../theme/deck.css`をそのまま使い、第2回だけの部品は`slides.md`の`style:`に置く。

## ファイル

| ファイル | 中身 |
|---|---|
| `slides.md` | Marp本体（全35枚） |
...
| 範囲 | 内容 | 時間 |
|---|---|---:|
| 1–5 | 導入。設定で結果が変わる例、4章の予告、CLIエージェントの定義 | 4分 |
| 6–10 | 1. コンテキスト。開始時点の中身、積み上がり、圧縮 | 8分 |
| 実演1 | 自分のClaude Codeで`/context`を見せる | 3分 |
| 11–13 | CLAUDE.md、rules、memory | 6分 |
| 14–17 | 2. エージェントループ、完了条件、ループエンジニアリング | 6分 |
| 18–22 | 3. ハーネス、事前と事後、skills、hooks | 8分 |
| 23–24 | hookが止めた実例（スクリーンショット）と、hookの中身 | 3分 |
| 実演2 | 23枚目の実例を自分の環境で再現する | 3分 |
| 25–26 | settings.json、確実に守らせたいものほど下の段へ | 3分 |
| 27–33 | 4. 私の環境。全体像、考え方4つ、ハーネスを作る順番 | 9分 |
| 34–35 | 用語の一覧、今日から試す3つ | 3分 |
| 質疑 | 途中で扱えなかった質問 | 5分 |

本編49分、実演6分、質疑5分。
時間が押した場合は、17枚目（ループエンジニアリング）と34枚目（用語の一覧）を口頭の一言に縮める。実演2は23枚目のスクリーンショットで代替できる。
...
- 用語は投影面では日常の言葉で言い、正式名は括弧で添える。用語一覧（34枚目）が対応表
